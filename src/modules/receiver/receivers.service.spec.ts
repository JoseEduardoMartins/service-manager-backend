...
const createResponse = new Receiver({
  id: 1,
  userId: 1,
  taxId: 'teste-1',
});
...
const saveResponse = new Receiver({
  id: 1,
  userId: 1,
  taxId: 'teste-1',
});
...
describe('ReceiversService', () => {
  let citiesService: ReceiversService;
  let citiesRepository: Repository<Receiver>;
  ...
  beforeEach(async () => {
    ...
    citiesService = module.get<ReceiversService>(ReceiversService);
    citiesRepository = module.get<Repository<Receiver>>(
      getRepositoryToken(Receiver),
    );
  });
